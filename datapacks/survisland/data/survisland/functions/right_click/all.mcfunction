...
execute if score #success survisland.data matches 0 store success score #success survisland.data if data entity @s Inventory[{Slot:-106b}].tag{CustomModelData:2010004} positioned ^ ^ ^2 as @p[gamemode=!spectator,tag=!survisland.temp,distance=..3] at @s run function survisland:utils/snuffer
