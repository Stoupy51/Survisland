...
execute if score #success survisland.data matches 1 unless entity @s[tag=survisland.has_idol] run tellraw @a[team=aMJ] ["\n",{"nbt":"SurvislandSpec","storage":"survisland:main","interpret":true},{"translate": "survisland.le_joueur"},{"selector":"@s","color":"aqua"},{"translate": "survisland.vient_de_rcuprer_un_idol_dans_son_inventaire"}]
